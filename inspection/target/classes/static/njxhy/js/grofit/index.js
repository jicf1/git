...
function initBtns() {
	...
	$(".form-btn").on('click', function() {
		window.location.href = $root() + "/router/grofit/form";
	})
	// 删除
	...
	$("#dtTable").on('click', '.donwload-btn', function() {
		...
		$ajax.get({
			url : '/api/grofit/downcheck?attaId=' + attaId,
			success : function (data) {
				if (data.success) {
					window.open($root() + '/api/grofit/download?attaId=' + attaId, "文件下载");
				} else {
